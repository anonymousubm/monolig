...
def sample_points_with_roi(
    rois, points, sample_radius_with_roi, num_max_points_of_part=200000
):
    """
    Args:
        rois: (M, 7 + C)
...
def sector_fps(points, num_sampled_points, num_sectors):
    ...
    sampled_points_batch_cnt = torch.tensor(
        num_sampled_points_list, device=points.device
    ).int()
...
class VoxelSetAbstraction(nn.Module):
        ...
        **kwargs,
        ...
        for src_name in self.model_cfg.FEATURES_SOURCE:
            ...
            if SA_cfg[src_name].get("INPUT_CHANNELS", None) is None:
                input_channels = (
                    SA_cfg[src_name].MLPS[0][0]
                    if isinstance(SA_cfg[src_name].MLPS[0], list)
                    else SA_cfg[src_name].MLPS[0]
                )
            else:
    ...
    def sectorized_proposal_centric_sampling(self, roi_boxes, points):
        ...
        sampled_points, _ = sample_points_with_roi(
            ...
            num_max_points_of_part=self.model_cfg.SPC_SAMPLING.get(
    ...
    def aggregate_keypoint_features_from_one_source(
        ...
        if filter_neighbors_with_roi:
            point_features = (
                torch.cat((xyz, xyz_features), dim=-1)
                if xyz_features is not None
                else xyz
            )
            point_features_list = []
            for bs_idx in range(batch_size):
            ...
            xyz_features = (
                valid_point_features[:, 3:] if xyz_features is not None else None
            )
        else:
            for bs_idx in range(batch_size):
    ...
    def forward(self, batch_dict):
        ...
        if "raw_points" in self.model_cfg.FEATURES_SOURCE:
            ...
            pooled_features = self.aggregate_keypoint_features_from_one_source(
                ...
                xyz=raw_points[:, 1:4],
                xyz_features=raw_points[:, 4:].contiguous()
                if raw_points.shape[1] > 4
                else None,
                xyz_bs_idxs=raw_points[:, 0],
                new_xyz=new_xyz,
                new_xyz_batch_cnt=new_xyz_batch_cnt,
                filter_neighbors_with_roi=self.model_cfg.SA_LAYER["raw_points"].get(
        ...
        for k, src_name in enumerate(self.SA_layer_names):
            ...
            pooled_features = self.aggregate_keypoint_features_from_one_source(
                ...
                new_xyz=new_xyz,
                new_xyz_batch_cnt=new_xyz_batch_cnt,
                filter_neighbors_with_roi=self.model_cfg.SA_LAYER[src_name].get(
                    "FILTER_NEIGHBOR_WITH_ROI", False
                ),
