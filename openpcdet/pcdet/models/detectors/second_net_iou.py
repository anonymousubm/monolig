...
class SECONDNetIoU(Detector3DTemplate):
    ...
    @staticmethod
    def cal_scores_by_npoints(
        cls_scores, iou_scores, num_points_in_gt, cls_thresh=10, iou_thresh=100
    ):
        """
        Args:
            cls_scores: (N)
    ...
    def post_processing(self, batch_dict):
        ...
        for index in range(batch_size):
            ...
            if post_process_cfg.NMS_CONFIG.MULTI_CLASSES_NMS:
                ...
                if (
                    post_process_cfg.NMS_CONFIG.get("SCORE_BY_CLASS", None)
                    and post_process_cfg.NMS_CONFIG.SCORE_TYPE == "score_by_class"
                ):
                    nms_scores = self.set_nms_score_by_class(
                        iou_preds,
                        cls_preds,
                        label_preds,
                        post_process_cfg.NMS_CONFIG.SCORE_BY_CLASS,
                    )
                elif (
                    ...
                    nms_scores = (
                        post_process_cfg.NMS_CONFIG.SCORE_WEIGHTS.iou * iou_preds
                    ...
                    nms_scores = self.cal_scores_by_npoints(
                        ...
                        iou_preds,
                        num_pts_in_gt,
                        score_thresh_cfg.cls,
                        score_thresh_cfg.iou,
                    )
                else:
                    raise NotImplementedError
