...
class RoIHeadTemplate(nn.Module):
    def __init__(self, num_class, model_cfg, **kwargs):
        ...
        self.uncertainty = "UNCERTAINTY" in model_cfg and model_cfg.UNCERTAINTY
    ...
    def build_losses(self, losses_cfg):
        if self.uncertainty:
            self.add_module(
                "reg_loss_func",
                loss_utils.WeightedSmoothL1LossUncertainty(
                    code_weights=losses_cfg.LOSS_WEIGHTS["code_weights"],
                    beta = losses_cfg.BETA,
                ),
            )
        else:
            self.add_module(
    ...
    def get_box_reg_layer_loss(self, forward_ret_dict):
        ...
        if loss_cfgs.REG_LOSS == "smooth-l1":
            ...
            if self.uncertainty:
                unc_reg = forward_ret_dict["unc_reg"]  # (rcnn_batch_size, 2)
                rcnn_loss_reg = self.reg_loss_func(
                    rcnn_reg.view(rcnn_batch_size, -1).unsqueeze(dim=0),
                    unc_reg.view(rcnn_batch_size, -1).unsqueeze(dim=0),
