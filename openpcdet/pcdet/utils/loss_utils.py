...
class WeightedSmoothL1LossUncertainty(nn.Module):
    """
    ...
    def __init__(self, beta: float = 1.0 / 9.0, code_weights: list = None):
        ...
        Args:
            ...
            code_weights: (#codes) float list if not None.
                Code-wise weights.
        """
        super(WeightedSmoothL1LossUncertainty, self).__init__()
        self.beta = beta
    ...
    def _gaussian_dist_loss(self, val, mean, var):
        gaussian_dist = self._gaussian_dist_pdf(val, mean, var)
        return - torch.log(gaussian_dist + 1e-9) / 2.0
    
    def _nll_loss(self, val, mean, var):
        term1 = (val - mean) ** 2.0 / (2.0 * var)
        term2 = torch.log(var + 1e-9) / 2.0
        return term1 + term2

    def forward(
        self, input: torch.Tensor, uncertainty: torch.Tensor, target: torch.Tensor, weights: torch.Tensor = None
    ):
        """
        Args:
            input: (B, #anchors, #codes) float tensor.
                Ecoded predicted locations of objects.
            uncertainty: (B, #anchors, 2) float tensor.
        ...
        loss_xy = self._gaussian_dist_loss(input[..., :2], target[..., :2], uncertainty)
        # loss_xy_nll = self._nll_loss(input[..., :2], target[..., :2], uncertainty)
        loss_rest = torch.abs(input[...,2:] - target[...,2:])
        
        loss = torch.cat([loss_xy, loss_rest],2)
