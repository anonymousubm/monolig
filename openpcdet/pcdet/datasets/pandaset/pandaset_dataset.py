...
class PandasetDataset(DatasetTemplate):
    ...
    def include_pandaset_infos(self, mode):
        ...
        if self.logger is not None:
            self.logger.info(
                "Total samples for PandaSet dataset ({}): {}".format(
                    self.mode, len(pandaset_infos)
                )
            )
    ...
    def _get_lidar_points(self, info, pose):
        ...
        return np.append(ego_points, np.expand_dims(points_int, axis=1), axis=1).astype(
            np.float32
        )
    ...
    def _get_annotations(self, info, pose):
        ...
        labels = np.array(
            [self.dataset_cfg.TRAINING_CATEGORIES.get(lab, lab) for lab in labels]
        )
        ...
        ego_boxes = np.vstack(
            [ego_xs, ego_ys, ego_zs, ego_dxs, ego_dys, ego_dzs, ego_yaws]
        ).T
    ...
    def generate_prediction_dicts(
        ...
        def generate_single_sample_dataframe(
            batch_index, box_dict, zrot_world_to_ego, pose
        ):
        ...
        for index, box_dict in enumerate(pred_dicts):
            ...
            single_pred_df = generate_single_sample_dataframe(
    ...
    def get_infos(self):
        ...
        for seq in self.sequences:
            ...
            if len(s.lidar.data) > 100:
                raise ValueError(
                    "The implementation for this dataset assumes that each sequence is "
                    + "no longer than 100 frames. The current sequence has {}".format(
                        len(s.lidar.data)
                    )
                )
            info = [
    ...
    def create_groundtruth_database(
        ...
        db_info_save_path = os.path.join(
            self.root_path, "pandaset_dbinfos_{}.pkl".format(split)
        )
    ...
    def evaluation(self, det_annos, class_names, **kwargs):
        self.logger.warning(
            "Evaluation is not implemented for Pandaset as there is no official one. "
...
def create_pandaset_infos(dataset_cfg, class_names, data_path, save_path):
    ...
    for split in ["train", "val", "test"]:
        print(
            "---------------- Start to generate {} data infos ---------------".format(
                split
            )
        )
    ...
    dataset.create_groundtruth_database(
        os.path.join(save_path, "pandaset_infos_train.pkl"), split="train"
    )
