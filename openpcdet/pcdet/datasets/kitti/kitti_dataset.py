...
class KittiDataset(DatasetTemplate):
    ...
    def generate_prediction_dicts(
        ...
        def generate_single_sample_dict(batch_index, box_dict):
            ...
            pred_unc = torch.sigmoid(box_dict["pred_unc"]).cpu().numpy()
            ...
            pred_dict["unc"] = pred_unc

            return pred_dict

        unc_output_dir = output_path.parents[0] / "aleatorics"
        unc_output_dir.mkdir(parents=True, exist_ok=True)
        annos = []
        for index, box_dict in enumerate(pred_dicts):
            ...
            if output_path is not None:
                cur_unc_file = unc_output_dir / ("%s.txt" % frame_id)
                with open(cur_unc_file,"w") as f:
                    unc = single_pred_dict["unc"]
                    for idx in range(len(unc)):
                        print("{:.4f} {:.4f}".format(unc[idx][0],unc[idx][1]),file=f)
...
def create_kitti_infos(dataset_cfg, class_names, data_path, save_path, workers=4):
    ...
    train_split, val_split, unused_split = "train", "val", "unused"

    train_filename = save_path / ("kitti_infos_%s.pkl" % train_split)
    val_filename = save_path / ("kitti_infos_%s.pkl" % val_split)
    unused_filename = save_path / ("kitti_infos_%s.pkl" % unused_split)
    ...
    dataset.set_split(unused_split)
    kitti_infos_unused = dataset.get_infos(
        num_workers=workers, has_label=True, count_inside_pts=True
    )
    with open(unused_filename, "wb") as f:
        pickle.dump(kitti_infos_unused, f)
    print("Kitti info unused file is saved to %s" % unused_filename)
...
if __name__ == "__main__":
    ...
    if sys.argv.__len__() > 1 and sys.argv[1] == "create_kitti_infos":
        ...
        path_to_dataset_folder = sys.argv[3]
        ROOT_DIR = (Path(__file__).resolve().parent / "../../../").resolve()
        create_kitti_infos(
            dataset_cfg=dataset_cfg,
            class_names=["Car", "Pedestrian", "Cyclist"],
            data_path=Path(path_to_dataset_folder),
            save_path=Path(path_to_dataset_folder),
        )
