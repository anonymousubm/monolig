...
class DataBaseSampler(object):
    def __init__(self, root_path, sampler_cfg, class_names, logger=None):
        ...
        self.gt_database_data_key = (
            self.load_db_to_shared_memory() if self.use_shared_memory else None
        )
    ...
    def load_db_to_shared_memory(self):
        ...
        cur_rank, world_size, num_gpus = common_utils.get_dist_info(
            return_gpu_per_machine=True
        )

        assert (
            self.sampler_cfg.DB_DATA_PATH.__len__() == 1
    ...
    def filter_by_difficulty(self, db_infos, removed_difficulty):
        ...
        for key, dinfos in db_infos.items():
            ...
            if self.logger is not None:
                self.logger.info(
                    "Database filter by difficulty %s: %d => %d"
    ...
    def filter_by_min_points(self, db_infos, min_gt_points_list):
        for name_num in min_gt_points_list:
            ...
            if min_num > 0 and name in db_infos.keys():
                ...
                if self.logger is not None:
                    self.logger.info(
                        "Database filter by min points %s: %d => %d"
    ...
    def add_sampled_boxes_to_scene(
        ...
        for class_name, sample_group in self.sample_groups.items():
            ...
            if int(sample_group["sample_num"]) > 0:
                ...
                sampled_boxes = np.stack(
                    [x["box3d_lidar"] for x in sampled_dict], axis=0
                ).astype(np.float32)
