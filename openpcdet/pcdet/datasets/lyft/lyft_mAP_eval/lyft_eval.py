...
class Box3D:
    ...
    def calculate_ground_bbox_coords(self):
        ...
        point_0_x = (
            self.center_x + self.length / 2 * cos_angle + self.width / 2 * sin_angle
        )
        point_0_y = (
            self.center_y + self.length / 2 * sin_angle - self.width / 2 * cos_angle
        )
...
if __name__ == "__main__":
    ...
    average_precisions = get_average_precisions(
        gt, predictions, class_names, args.iou_threshold
    )
