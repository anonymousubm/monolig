...
class LyftDataset(DatasetTemplate):
    ...
    def remove_ego_points(points, center_radius=1.0):
        mask = ~(
            (np.abs(points[:, 0]) < center_radius * 1.5)
    ...
    def get_sweep(self, sweep_info):
        ...
        if points_sweep.shape[0] % 5 != 0:
            points_sweep = points_sweep[
                : points_sweep.shape[0] - (points_sweep.shape[0] % 5)
            ]
    ...
    def lyft_eval(self, det_annos, class_names, iou_thresholds=[0.5]):
        ...
        lyft = Lyft(
            json_path=self.root_path / "data", data_path=self.root_path, verbose=True
        )
        ...
        average_precisions = get_average_precisions(
            gt_lyft_boxes, det_lyft_boxes, class_names, iou_thresholds
        )
...
def create_lyft_info(version, data_path, save_path, split, max_sweeps=10):
    ...
    train_scenes = (
        [x.strip() for x in open(train_split_path).readlines()]
        if train_split_path.exists()
        else []
    )
    val_scenes = (
        [x.strip() for x in open(val_split_path).readlines()]
        if val_split_path is not None and val_split_path.exists()
        else []
    )
    ...
    if version == "test":
        ...
        print(
            ...
            % (len(train_lyft_infos), len(val_lyft_infos))
        )
