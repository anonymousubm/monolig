...
def fill_trainval_infos(
    ...
    progress_bar = tqdm.tqdm(
        total=len(lyft.sample), desc="create_info", dynamic_ncols=True
    )
...
def convert_det_to_lyft_format(lyft, det_annos):
    ...
    for anno in det_annos:
        ...
        boxes_lyft_list = boxes_lidar_to_lyft(
            anno["boxes_lidar"], anno["score"], anno["pred_labels"]
        )
        boxes_list = lidar_lyft_box_to_global(
            lyft, boxes_lyft_list, anno["metadata"]["token"]
        )
