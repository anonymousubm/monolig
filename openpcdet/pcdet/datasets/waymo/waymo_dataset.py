...
class WaymoDataset(DatasetTemplate):
    ...
    def create_groundtruth_database(
        ...
        database_save_path = save_path / (
            "%s_gt_database_%s_sampled_%d"
            % (processed_data_tag, split, sampled_interval)
        )
        db_info_save_path = save_path / (
            "%s_waymo_dbinfos_%s_sampled_%d.pkl"
            % (processed_data_tag, split, sampled_interval)
        )
        db_data_save_path = save_path / (
            "%s_gt_database_%s_sampled_%d_global.npy"
            % (processed_data_tag, split, sampled_interval)
        )
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--processed_data_tag", type=str, default="waymo_processed_data_v0_5_0", help=""
    )
