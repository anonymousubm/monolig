...
class OpenPCDetWaymoDetectionMetricsEstimator(tf.test.TestCase):
    ...
    def generate_waymo_type_results(
        ...
        def boxes3d_kitti_fakelidar_to_lidar(boxes3d_lidar):
            ...
            return np.concatenate(
                [boxes3d_lidar[:, 0:3], l, w, h, -(r + np.pi / 2)], axis=-1
            )
        ...
        for frame_index, info in enumerate(infos):
            if is_gt:
                box_mask = np.array(
                    [n in class_names for n in info["name"]], dtype=np.bool_
                )
                ...
                if fake_gt_infos:
                    info["gt_boxes_lidar"] = boxes3d_kitti_fakelidar_to_lidar(
                        info["gt_boxes_lidar"]
                    )
    ...
    def waymo_evaluation(
        ...
        assert len(prediction_infos) == len(gt_infos), "%d vs %d" % (
            prediction_infos.__len__(),
            gt_infos.__len__(),
        )
        ...
        (
            ...
            pd_overlap_nlz,
            _,
        ) = self.generate_waymo_type_results(prediction_infos, class_name, is_gt=False)
        (
            gt_frameid,
            gt_boxes3d,
            gt_type,
...
def main():
    ...
    parser.add_argument(
        ...
        type=str,
        nargs="+",
        default=["Vehicle", "Pedestrian", "Cyclist"],
        help="",
    )
    parser.add_argument(
        "--sampled_interval",
        type=int,
        default=5,
        help="sampled interval for GT sequences",
    )
