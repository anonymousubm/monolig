...
def get_latest_checkpoint(path_to_experiments):
    checkpoints_list = glob.glob(os.path.join(path_to_experiments, "ckpt", "*.pth"))
...
def parse_config():
    ...
    parser.add_argument(
        "--path_to_experiments", type=str, default=None, help="path to experiments"
    )
    ...
    cfg.ROOT_DIR = Path("/path/to/experiments")
    cfg.EXPERIMENT_FOLDER = Path(args.path_to_experiments)
    np.random.seed(1024)

    if args.set_cfgs is not None:
        cfg_from_list(args.set_cfgs, cfg)
    if args.ckpt is None:
        args.ckpt = get_latest_checkpoint(args.path_to_experiments)
    return args, cfg
...
def main():
    ...
    output_dir = cfg.EXPERIMENT_FOLDER
