...
from active_learning.modelling.loss_prediction import LossPrediction, build_loss_prediction_optimizer, LossPredLoss
...
def main(cfg):
    ...
    if "CONTINUE_OPTIMIZATION" in cfg["SOLVER"]:
        continue_optimization = cfg["SOLVER"]["CONTINUE_OPTIMIZATION"]
    if continue_optimization:
        print("Optimization continuing from last checkpoint")
        ...
        print("Optimization starting from scratch")
        model = build_model(cfg)
    ...
    if comm.is_distributed():
        ...
        sync_bn_pg = (
            d2_comm._LOCAL_PROCESS_GROUP
            if cfg.SOLVER.SYNCBN_USE_LOCAL_WORKERS
            else None
        )
    ...
    do_train(cfg, model, optimizer, scheduler, checkpointer)
    test_results = do_test(cfg, model, is_last=True)
    if cfg.TEST.AUG.ENABLED:
        test_results.update(do_test(cfg, model, is_last=True, use_tta=True))
    ...
    if ("LOSS_MODULE" in cfg["MODEL"]) and cfg["MODEL"]["LOSS_MODULE"]:
        ...
        optimizer_loss_pred = build_loss_prediction_optimizer(model_loss_pred)
        checkpointer_loss_pred = Checkpointer(model_loss_pred, "./", optimizer=optimizer_loss_pred)
        checkpoint_file = cfg.MODEL.CKPT
        loss_checkpoint_file = "/"+os.path.join(*(checkpoint_file.split("/")[:-1]+["loss_"+checkpoint_file.split("/")[-1]]))
        if os.path.exists(loss_checkpoint_file):
            checkpointer_loss_pred.load(loss_checkpoint_file)
        do_train_loss(cfg, model, model_loss_pred, optimizer_loss_pred, checkpointer_loss_pred)
    return test_results, cfg

def do_train_loss(cfg, model, model_loss_pred, optimizer_loss_pred, checkpointer_loss_pred):
    max_iter = 1000
    train_mapper = get_dataset_mapper(cfg, is_train=True)
    dataloader, _ = build_train_dataloader(cfg, mapper=train_mapper)

    model_loss_pred.train()
    optimizer_loss_pred.zero_grad()
    for data, _ in tqdm(zip(dataloader, range(max_iter)), total=max_iter):
        loss_loss_pred = model_loss_pred(data, model)
        loss_loss_pred.backward()
        optimizer_loss_pred.step()
        optimizer_loss_pred.zero_grad()
    checkpointer_loss_pred.save("loss_model")

def do_train(cfg, model, optimizer, scheduler, checkpointer):
    model.train()
    ...
    for data, iteration in zip(dataloader, range(max_iter * accumulate_grad_batches)):
        ...
        loss_dict = {
            name: loss / accumulate_grad_batches for name, loss in loss_dict.items()
        }
        ...
        loss_dict_reduced = {
            k: v.item() for k, v in d2_comm.reduce_dict(loss_dict).items()
        }
        assert torch.isfinite(
            torch.as_tensor(list(loss_dict_reduced.values()))
        ).all(), loss_dict_reduced
        ...
        if batch_iter > 5 and (batch_iter % 20 == 0 or batch_iter == max_iter):
            ...
            if d2_comm.is_main_process() and cfg.WANDB.ENABLED:
                wandb.log(
                    {"epoch": 1 + num_images_seen // len(dataset_dicts)},
                    step=batch_iter,
                )
                wandb.log({"num_images_seen": num_images_seen}, step=batch_iter)

        if (
            cfg.VIS.DATALOADER_ENABLED
            and batch_iter % cfg.VIS.DATALOADER_PERIOD == 0
            and d2_comm.is_main_process()
        ):
            ...
            if cfg.WANDB.ENABLED:
                per_image_vis = [
                    mosaic(list(viz_images[idx].values())) for idx in range(len(data))
                ]
                wandb.log(
                    {
                        "dataloader": [
                            wandb.Image(vis, caption=f"idx={idx}")
                            for idx, vis in enumerate(per_image_vis)
                        ]
                    },
                    step=batch_iter,
                )
        ...
        if d2_comm.is_main_process():
            periodic_checkpointer.step(
                batch_iter - 1
            )  # (fvcore) model_0004999.pth checkpoints 5000-th iteration

        if (
            cfg.SYNC_OUTPUT_DIR_S3.ENABLED
            and batch_iter > 0
            and batch_iter % cfg.SYNC_OUTPUT_DIR_S3.PERIOD == 0
        ):
            sync_output_dir_s3(cfg)

        if (
            cfg.TEST.EVAL_PERIOD > 0
            and batch_iter % cfg.TEST.EVAL_PERIOD == 0
            and batch_iter != max_iter
        ) or batch_iter in cfg.TEST.ADDITIONAL_EVAL_STEPS:
            do_test(cfg, model)
            d2_comm.synchronize()
