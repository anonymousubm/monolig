...
from tridet.data.datasets.kitti_3d.build import (
...
DATASET_DICTS_BUILDER = {
    ...
    "kitti_3d_train_project_box3d": (
        build_monocular_kitti3d_dataset,
        dict(mv3d_split="train", box2d_from_box3d=True),
    ),
    "kitti_3d_train_right_cam": (
        build_monocular_kitti3d_dataset,
        dict(mv3d_split="train", sensors=("camera_3",)),
    ),
    "kitti_3d_train_both_cams": (
        build_monocular_kitti3d_dataset,
        dict(mv3d_split="train", sensors=("camera_2", "camera_3")),
    ),
    ...
    "kitti_3d_overfit": (
        build_monocular_kitti3d_dataset,
        dict(mv3d_split="train", max_num_items=32),
    ),
    "kitti_3d_unused": (build_monocular_kitti3d_dataset, dict(mv3d_split="unused")),
}

METADATA_BUILDER = {
    name: (register_kitti_3d_metadata, {}) for name in DATASET_DICTS_BUILDER.keys()
}


def register_kitti_3d_datasets(required_datasets, cfg):
    kitti_3d_datasets = sorted(
        list(set(required_datasets).intersection(DATASET_DICTS_BUILDER.keys()))
    )
