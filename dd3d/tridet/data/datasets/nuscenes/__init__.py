...
DATASET_DICTS_BUILDER = {
    name: (build_nuscenes_dataset, dict(name=name)) for name in NUSC_DATASET_NAMES
}

METADATA_BUILDER = {
    name: (register_nuscenes_metadata, {}) for name in DATASET_DICTS_BUILDER.keys()
}
...
def register_nuscenes_datasets(required_datasets, cfg):
    if (
        ...
        in ("nusc_train", "nusc_val", "nusc_trainval", "nusc_test")
        and get_world_size() > 1
    ):
        raise LOG.warning(
            "The distributed evaluation does not work well with large test set for now. "
            f"If program hangs, consider using non-distributed evaluation: {cfg.DATASETS.TEST.NAME}"
        )

    nusc_datasets = sorted(
