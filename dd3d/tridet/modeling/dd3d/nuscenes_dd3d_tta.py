...
from tridet.modeling.dd3d.test_time_augmentation import DatasetMapperTTA
...
class NuscenesDD3DWithTTA(nn.Module):
    def __init__(self, cfg, model, tta_mapper=None):
        ...
        assert isinstance(
            model, NuscenesDD3D
        ), "NuscenesDD3DWithTTA only supports on NuscenesDD3D. Got a model of type {}".format(
            type(model)
        )
    ...
    def __call__(self, batched_inputs):
        ...
        instances_per_image = [
            self._inference_one_image(_maybe_read_image(x)) for x in batched_inputs
        ]
    ...
    def _inference_one_image(self, x):
        ...
        merged_instances = self._get_augmented_instances(
            augmented_inputs, tfms, orig_shape
        )
    ...
    def _get_augmented_instances(self, augmented_inputs, tfms, orig_shape):
        ...
        for input, output, tfm in zip(augmented_inputs, outputs, tfms):
            ...
            orig_vec_pred_boxes_3d = [
                inv_tfm.apply_box3d(box3d_as_vec)
                for box3d_as_vec in vectorized_boxes_3d
            ]

            # intrinsics
            orig_intrinsics = inv_tfm.apply_intrinsics(
                input["intrinsics"].cpu().numpy()
            )
