...
from tridet.utils.tensor2d import (
...
class DD3D(nn.Module):
    def __init__(self, cfg):
        ...
        self.backbone_output_shape = [
            backbone_output_shape[f] for f in self.in_features
        ]
        ...
        self.weight_labeled = cfg.MODEL.WEIGHT_LABELED
        self.weight_unlabeled = cfg.MODEL.WEIGHT_UNLABELED

        # nuScenes inference aggregates detections over all 6 cameras.
        self.nusc_sample_aggregate_in_inference = (
            cfg.DD3D.INFERENCE.NUSC_SAMPLE_AGGREGATE
        )
    ...
    def forward(self, batched_inputs):
        ...
        if "instances" in batched_inputs[0]:
            ...
            labeled_flags = [x["labeled_flag"] for x in batched_inputs]
            labeled_weights = [
                self.weight_labeled if f else self.weight_unlabeled
                for f in labeled_flags
            ]
            gt_weights = [
                torch.Tensor(x["weights"]).to(self.device) for x in batched_inputs
            ]
            gt_weights = [
                w * l for w, l in zip(gt_weights, labeled_weights)
            ]
        else:
            gt_instances = None
        ...
        if self.training:
            ...
            training_targets = self.prepare_targets(
                locations, gt_instances, feature_shapes, gt_weights, labeled_weights
            )
