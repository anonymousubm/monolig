...
def predictions_to_boxes3d(
    ...
    if depth_is_distance:
        depth = depth / unproject_points2d(locations, inv_intrinsics).norm(dim=1).clamp(
            min=EPS
        )
...
class FCOS3DHead(nn.Module):
    def __init__(self, cfg, input_shape):
        ...
        self.register_buffer(
            "mean_depth_per_level", torch.Tensor(cfg.DD3D.FCOS3D.MEAN_DEPTH_PER_LEVEL)
        )
        self.register_buffer(
            "std_depth_per_level", torch.Tensor(cfg.DD3D.FCOS3D.STD_DEPTH_PER_LEVEL)
        )
        ...
        num_classes = (
            self.num_classes if not cfg.DD3D.FCOS3D.CLASS_AGNOSTIC_BOX3D else 1
        )
...
class FCOS3DLoss:
    def __init__(self, cfg):
        ...
        self.scale_depth_by_focal_lengths_factor = (
            cfg.DD3D.FCOS3D.SCALE_DEPTH_BY_FOCAL_LENGTHS_FACTOR
        )
        self.predict_distance = cfg.DD3D.FCOS3D.PREDICT_DISTANCE

        self.box3d_reg_loss_fn = DisentangledBox3DLoss(
            cfg
    ...
    def __call__(
        ...
        losses_box3d, box3d_l1_error = self.box3d_reg_loss_fn(
            box3d_pred, box3d_targets, locations, weights * weights * centerness_targets
        )
        ...
        loss_conf3d = (
            ...
            * (loss_conf3d * weights * weights * centerness_targets).sum()
            / loss_denom
        )
...
class FCOS3DInference:
    ...
    def __call__(
        ...
        for lvl, (
            ...
            box3d_size_lvl,
            box3d_conf_lvl,
        ) in enumerate(zip(box3d_quat, box3d_ctr, box3d_depth, box3d_size, box3d_conf)):
    ...
    def forward_for_single_feature_map(
        ...
        box3d_conf = (
            box3d_conf.permute(0, 2, 3, 1).reshape(N, -1, num_classes).sigmoid()
        )
        ...
        for i in range(N):
            ...
            if self.class_agnostic:
                ...
                box3d_quat_per_im = torch.gather(
                    box3d_quat_per_im, dim=2, index=I.repeat(1, 4, 1)
                ).squeeze(-1)
                box3d_ctr_per_im = torch.gather(
                    box3d_ctr_per_im, dim=2, index=I.repeat(1, 2, 1)
                ).squeeze(-1)
                box3d_depth_per_im = torch.gather(
                    box3d_depth_per_im, dim=1, index=I.squeeze(-1)
                ).squeeze(-1)
                box3d_size_per_im = torch.gather(
                    box3d_size_per_im, dim=2, index=I.repeat(1, 3, 1)
                ).squeeze(-1)
                box3d_conf_per_im = torch.gather(
                    box3d_conf_per_im, dim=1, index=I.squeeze(-1)
                ).squeeze(-1)
            ...
            canon_box_sizes = box3d_quat.new_tensor(self.canon_box_sizes)[
                pred_instances[i].pred_classes
            ]
