...
class FCOS2DLoss:
    ...
    def __call__(self, logits, box2d_reg, centerness, targets):
        ...
        loss_cls = weights * loss_cls.T
        loss_cls = loss_cls.sum() / num_pos_avg
        ...
        ## (aral) centerness_targets is the likelihood of this object being center, meaning, left and right should be close, also top and bottom.
        ## (aral) that's why they look at the min(lr)/max(lr).
        centerness_targets = compute_ctrness_targets(box2d_reg_targets)

        # Denominator for all foreground losses.
        ## (aral) This is kind of like weighting different instances differently.
        ## (aral) if it has a high weight and center, take its IoU into more consideration.
        ctrness_targets_sum = (weights * centerness_targets).sum()
        ...
        loss_box2d_reg = (
            self.box2d_reg_loss_fn(
                box2d_reg_pred,
                box2d_reg_targets,
                weights * weights * centerness_targets,
            )
            / loss_denom
        )
        ...
        loss_centerness = F.binary_cross_entropy_with_logits(
            centerness_pred, centerness_targets, reduction="none"
        )
        loss_centerness = weights * loss_centerness
        loss_centerness = loss_centerness.sum() / num_pos_avg
...
class FCOS2DInference:
    ...
    def __call__(self, logits, box2d_reg, centerness, locations, image_sizes):
        ...
        for lvl, (
            ...
            centerness_lvl,
            locations_lvl,
        ) in enumerate(zip(logits, box2d_reg, centerness, locations)):
            ...
            for instances_per_im in instances_per_lvl:
                instances_per_im.fpn_levels = (
                    locations_lvl.new_ones(len(instances_per_im), dtype=torch.long)
                    * lvl
                )
    ...
    def forward_for_single_feature_map(
        ...
        for i in range(N):
            ...
            if candidate_mask_per_im.sum().item() > pre_nms_topk_per_im.item():
                scores_per_im, topk_indices = scores_per_im.topk(
                    pre_nms_topk_per_im, sorted=False
                )
        ...
        for instances in instances_per_im:
            if self.nms_thresh > 0:
                ...
                keep = batched_nms(
                    ...
                    instances.get(score_key_for_nms),
