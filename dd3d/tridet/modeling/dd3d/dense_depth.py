...
class DD3DDenseDepthHead(nn.Module):
    def __init__(self, cfg, input_shape):
        ...
        self.mean_depth_per_level = torch.FloatTensor(
            cfg.DD3D.FCOS3D.MEAN_DEPTH_PER_LEVEL
        )
        self.std_depth_per_level = torch.FloatTensor(
            cfg.DD3D.FCOS3D.STD_DEPTH_PER_LEVEL
        )
        ...
        self.dense_depth = nn.ModuleList(
            [
                Conv2d(
                    ...
                    stride=1,
                    padding=1,
                    bias=(not cfg.DD3D.FCOS3D.USE_SCALE),
                )
    ...
    def forward(self, x):
        ...
        for l, features in enumerate(x):
            ...
            if self.use_scale:
                dense_depth_lvl = self.offsets_depth[l](
                    self.scales_depth[l](dense_depth_lvl)
                )
...
class DD3DDenseDepth(nn.Module):
    ...
    def forward(self, batched_inputs):
        ...
        dense_depth = [
            aligned_bilinear(
                x, factor=stride, offset=self.feature_locations_offset
            ).squeeze(1)
