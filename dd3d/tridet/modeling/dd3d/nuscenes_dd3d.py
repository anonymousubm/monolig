...
from tridet.modeling.dd3d.postprocessing import (
    get_group_idxs,
...
class NuscenesDD3DTargetPreparer(DD3DTargetPreparer):
    ...
    def __call__(self, locations, gt_instances, feature_shapes):
        ...
        box2d_reg_targets = cat(
            [x.reshape(-1, 4) for x in training_targets["box2d_reg"]]
        )
    ...
    def compute_targets_for_locations(
        ...
        for im_i in range(len(targets)):
            ...
            box2d_reg_per_im = box2d_reg_per_im[
...
class NuscenesLoss:
    ...
    def __call__(self, attr_logits, speeds, fcos2d_info, targets):
        # Flatten predictions
        attr_logits = cat(
            [x.permute(0, 2, 3, 1).reshape(-1, MAX_NUM_ATTRIBUTES) for x in attr_logits]
        )
        ...
        if pos_inds.numel() == 0:
            losses = {
                "loss_attr": attr_logits.sum() * 0.0,
                "loss_speed": speeds.sum() * 0.0,
            }
        ...
        valid_attr_mask = (
            target_attr != MAX_NUM_ATTRIBUTES
        ...
        attr_loss_denom = max(
            reduce_sum(attr_weights_sum).item() / d2_comm.get_world_size(), 1e-6
        )

        if valid_attr_mask.sum() == 0:
            losses.update({"loss_attr": attr_logits.sum() * 0.0})
        else:
        ...
        speed_loss_denom = max(
            reduce_sum(speed_weights_sum).item() / d2_comm.get_world_size(), 1e-6
        )

        if valid_gt_mask.sum() == 0:
            losses.update({"loss_speed": speeds.sum() * 0.0})
            # return losses
        else:
...
class NuscenesInference:
    ...
    def __call__(self, attr_logits, speeds, pred_instances, fcos2d_info):
        ...
        for lvl, (attr_logits_lvl, speed_lvl, info_lvl, instances_lvl) in enumerate(
            zip(attr_logits, speeds, fcos2d_info, pred_instances)
        ):

            attr_logits_lvl = attr_logits_lvl.permute(0, 2, 3, 1).reshape(
                N, -1, MAX_NUM_ATTRIBUTES
            )
            ...
            for i in range(N):
                ...
                if len(attr_logits_per_im) == 0:
                    instances_lvl[i].pred_attributes = instances_lvl[
                        i
                    ].pred_classes.new_tensor([])
...
class NuscenesDD3D(DD3D):
    ...
    def forward(self, batched_inputs):
        ...
        images = ImageList.from_tensors(
            images, self.backbone.size_divisibility, intrinsics=intrinsics
        )
        ...
        inv_intrinsics = (
            images.intrinsics.inverse() if images.intrinsics is not None else None
        )
