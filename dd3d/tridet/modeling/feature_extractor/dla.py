...
class DLA(Backbone):
    ...
    def __init__(
        ...
        self.level0 = self._make_conv_level(
            channels[0], channels[0], levels[0], norm=norm
        )
        self.level1 = self._make_conv_level(
            channels[0], channels[1], levels[1], stride=2, norm=norm
        )
        self.level2 = Tree(
            levels[2],
            block,
        ...
        self._out_feature_channels = {
            name: out_feature_channels[name] for name in self._out_features
        }
        self._out_feature_strides = {
            name: out_feature_strides[name] for name in self._out_features
        }
    ...
    def forward(self, x):
        assert (
            x.dim() == 4
        ), f"DLA takes an input of shape (N, C, H, W). Got {x.shape} instead!"
        outputs = {}
        x = self.base_layer(x)
        for i in range(6):
            name = f"level{i}"
