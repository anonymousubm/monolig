...
class BEVImage:
    ...
    def __init__(
        ...
        self._center_pixel = (
            int(metric_height * pixels_per_meter) // 2,
            int(metric_width * pixels_per_meter) // 2,
        )
    ...
    def __repr__(self):
        return "width: {}, height: {}, data: {}".format(
            self._metric_width, self._metric_height, type(self.data)
        )
    ...
    def reset(self):
        ...
        for i in range(
            1,
            int(max(self._metric_width, self._metric_height))
            // self._polar_step_size_meters,
        ):
            cv2.circle(
                self.data,
                self._center_pixel,
    ...
    def render_point_cloud(self, point_cloud, extrinsics=Pose(), color=GRAY):
        ...
        point_cloud2d[:, 0] = (
            self._center_pixel[0] + point_cloud2d[:, 0] * self._pixels_per_meter
        )
        point_cloud2d[:, 1] = (
            self._center_pixel[1] + point_cloud2d[:, 1] * self._pixels_per_meter
        )
    ...
    def render_radar_point_cloud(
        ...
        if velocity is not None:
            ...
            point_cloud2d_tail[:, 0] = (
                self._center_pixel[0]
                + point_cloud2d_tail[:, 0] * self._pixels_per_meter
            )
            point_cloud2d_tail[:, 1] = (
                self._center_pixel[1]
                + point_cloud2d_tail[:, 1] * self._pixels_per_meter
            )
    ...
    def render_bounding_box_3d(
        ...
        for bidx, (corners, color) in enumerate(zip(boxes_corners, colors)):
            ...
            corners2d[:, 0] = (
                self._center_pixel[0] + corners2d[:, 0] * self._pixels_per_meter
            )
            corners2d[:, 1] = (
                self._center_pixel[1] + corners2d[:, 1] * self._pixels_per_meter
            )
    ...
    def render_camera_frustrum(
        self, intrinsics, extrinsics, width, color=YELLOW, line_thickness=1
    ):
        """
        ...
        frustrum_in_bev[:, 0] = (
            self._center_pixel[0] + frustrum_in_bev[:, 0] * self._pixels_per_meter
        )
        frustrum_in_bev[:, 1] = (
            self._center_pixel[1] + frustrum_in_bev[:, 1] * self._pixels_per_meter
        )

        frustrum_in_bev[1:] = (
            100 * (frustrum_in_bev[1:] - frustrum_in_bev[0]) + frustrum_in_bev[0]
        )
        frustrum_in_bev = frustrum_in_bev.astype(np.int32)

        cv2.line(
            self.data,
            tuple(frustrum_in_bev[0]),
            tuple(frustrum_in_bev[1]),
            color,
            line_thickness,
        )
        cv2.line(
