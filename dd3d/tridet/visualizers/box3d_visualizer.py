...
from tridet.utils.visualization import (
...
def pretty_render_3d_box(
    ...
    *,
    ...
    if not isinstance(box3d, GenericBoxes3D):
        raise ValueError(
            f"`box3d` must be a type of `Genericboxes3D`: {type(box3d).__str__()}"
        )
    if (
    ...
    cv2.polylines(
        image, [corners.T[:4].astype(np.int32)], True, color, thickness=line_thickness
    )
    cv2.polylines(
        image, [corners.T[4:].astype(np.int32)], True, color, thickness=line_thickness
    )
    ...
    if render_label:
        ...
        label = _create_text_labels(
            [class_id], [score] if score is not None else None, class_names
        )[0]
        # bottom-right corner
        text_pos = tuple([points2d[:, 0].min(), points2d[:, 1].max()])
        horiz_align = "left"
        lighter_color = change_color_brightness(
            tuple([c / 255.0 for c in color]), brightness_factor=0.8
        )
        H, W = image.shape[:2]
        default_font_size = max(np.sqrt(H * W) // 90, 10)
        height_ratio = (points2d[:, 1].max() - points2d[:, 1].min()) / np.sqrt(H * W)
        font_size = (
            np.clip((height_ratio - 0.02) / 0.08 + 1, 1.2, 2) * 0.5 * default_font_size
        )
...
def draw_boxes3d_cam(
    img, boxes3d, class_ids, metadata, intrinsics=None, scores=None, render_labels=True
):
    """
    Parameters
    ----------
    ...
    assert len(class_ids) == len(
        boxes3d
    ), "Number of class IDs must be same with number of 3D boxes."
    ...
    if isinstance(boxes3d, Boxes3D):
        ...
        if intrinsics is not None:
            assert torch.allclose(
                _intrinsics, torch.as_tensor(intrinsics).reshape(3, 3)
            )
    ...
    class_names = [
        metadata.contiguous_id_to_name[class_id]
...
def draw_boxes3d_bev(
    ...
    if isinstance(extrinsics, np.ndarray):
        ...
        extrinsics = Pose(
            wxyz=np.array(extrinsics["wxyz"]), tvec=np.array(extrinsics["tvec"])
        )
    ...
    if point_cloud is not None:
        ...
        pc_extrinsics = Pose(
            wxyz=np.array(point_cloud_extrnsics[:4]),
            tvec=np.array(point_cloud_extrnsics[4:]),
        )
    ...
    bev.render_bounding_box_3d(
        boxes3d=boxes3d, extrinsics=extrinsics, colors=colors, line_thickness=5
    )
    ...
    def __init__(self, cfg, dataset_name, inference_output_dir):
        ...
        for dataset_dict in dataset_dicts:
            ...
            pred_instances_by_image[image_id] = create_instances(
                pred_instances_by_image[image_id],
                img_shape,
                det3d_threshold,
                self._metadata,
                score_key="score_3d",
...
class Box3DDataloaderVisualizer:
    ...
    def visualize(self, x):
        ...
        img = d2_utils.convert_image_to_rgb(
            x["image"].permute(1, 2, 0), self._input_format
        )
