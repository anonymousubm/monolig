...
def rotate_iou_gpu_eval(boxes, query_boxes, criterion=-1, device_id=0):
    ...
    with stream.auto_synchronize():
        ...
        rotate_iou_kernel_eval[
            blockspergrid,
            threadsPerBlock,  # pylint: disable=unsubscriptable-object
            stream,
...
def distance_kernel_eval(N, K, dev_loc, dev_query_loc, dev_distances, criterion=-1):
    ...
    if tx < row_size:
        for i in range(col_size):
            ...
            dev_distances[offset] = -1 * calculate_distance(
                block_qloc[i * 2 : i * 2 + 2], block_loc[tx * 2 : tx * 2 + 2]
            )
...
def neg_box_distance(loc, query_loc, criterion=-1, device_id=0):
    ...
    with stream.auto_synchronize():
        ...
        distance_kernel_eval[
            ...
            threadsPerBlock,  # pylint: disable=unsubscriptable-object
            stream,
        ](N, K, loc_dev, query_loc_dev, distances_dev, criterion)
