...
from tridet.data.datasets.nuscenes.build import (
    ATTRIBUTE_IDS,
    CATEGORY_IDS,
...
PED_ATTR_ID_TO_NAME = {
    ...
    2: "pedestrian.sitting_lying_down",
}
...
class DetectionEval(_DetectionEval):
    ...
    def __init__(self, nusc, config, result_path, eval_set, output_dir, verbose):
        ...
        # ----------------------------------------------------------------
        assert set(self.pred_boxes.sample_tokens).issubset(
            set(gt_boxes.sample_tokens)
...
class NuscenesEvaluator(DatasetEvaluator):
    ...
    def process(self, inputs, outputs):
        ...
        idx_to_token = get_group_idxs(
            sample_tokens, NUM_IMAGES_PER_SAMPLE, inverse=True
        )
        ...
        for image_idx, (input_per_image, pred_per_image) in enumerate(
            zip(inputs, outputs)
        ):
            ...
            for (
                ...
                vel_global = (
                    speed.cpu().numpy()
                    * Quaternion(box3d_global.quat.tolist()[0]).rotation_matrix.T[0]
                )
    ...
    def evaluate(self):
        ...
        nusc = NuScenes(
            version=DATASET_NAME_TO_VERSION[self._dataset_name],
