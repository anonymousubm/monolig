...
class KITTI3DEvaluator(DatasetEvaluator):
    ...
    def evaluate(self):
        ...
        if self._distributed:
            ...
            predictions_kitti_format = comm.gather(
                self._predictions_kitti_format, dst=0
            )
            predictions_kitti_format = list(itertools.chain(*predictions_kitti_format))

            groundtruth_kitti_format = comm.gather(
                self._groundtruth_kitti_format, dst=0
            )
        ...
        if self._only_prepare_submission:
            self.prepare_kitti3d_submission(
                ...
                submission_dir=os.path.join(self._output_dir, KITTI_SUBMISSION_DIR),
            )
            return {}

        assert len(predictions_kitti_format) == len(groundtruth_kitti_format)
        formatted_predictions = [
            KITTIEvaluationEngine._format(idx, x, True)
            for idx, x in enumerate(predictions_kitti_format)
        ]
        formatted_groundtruth = [
            KITTIEvaluationEngine._format(idx, x, False)
            for idx, x in enumerate(groundtruth_kitti_format)
        ]
        ...
        results = engine.evaluate(
            formatted_groundtruth,
            formatted_predictions,
            overlap_thresholds=self._iou_thresholds,
        )

        results = OrderedDict({k: 100.0 * v for k, v in results.items()})
    ...
    def prepare_kitti3d_submission(predictions_kitti_format, submission_dir):
        ...
        for idx, prediction in tqdm(enumerate(predictions_kitti_format)):
            prediction.to_csv(
                os.path.join(submission_dir, f"{idx:06d}.txt"),
                sep=" ",
                header=False,
                index=False,
            )
...
def convert_3d_box_to_kitti(box):
    ...
    if quat.axis[2] > 0:
        kitti_pose = Pose(
            wxyz=Quaternion(axis=[0, 1, 0], radians=-quat.angle), tvec=tvec
        )
        rot_y = -quat.angle
    else:
        kitti_pose = Pose(
...
class KITTIEvaluationEngine:
    ...
    def __init__(self, id_to_name, num_shards=50, sample_points=41):
        ...
        self.filter_data_fn = partial(
            clean_kitti_data,
            difficulty_level_to_params=self._DEFAULT_KITTI_LEVEL_TO_PARAMETER,
        )
    ...
    def evaluate(self, gt_annos, dt_annos, overlap_thresholds):
        ...
        box3d_pr_curves = self.eval_metric(
            gt_annos, dt_annos, "BOX3D_AP", overlap_thresholds
        )
        mAP_3d = self.get_mAP(box3d_pr_curves["precision"], box3d_pr_curves["recall"])

        bev_pr_curves = self.eval_metric(
            gt_annos, dt_annos, "BEV_AP", overlap_thresholds
        )
        ...
        for class_i, class_name in self.id_to_name.items():
            for diff_i, diff in enumerate(["Easy", "Moderate", "Hard"]):
                for thresh_i, thresh in enumerate(overlap_thresholds):
                    results[
                        "kitti_box3d_r40/{}_{}_{}".format(class_name, diff, thresh)
                    ] = mAP_3d[class_i, diff_i, thresh_i]
        for class_i, class_name in self.id_to_name.items():
            for diff_i, diff in enumerate(["Easy", "Moderate", "Hard"]):
                for thresh_i, thresh in enumerate(overlap_thresholds):
                    results[
                        "kitti_bev_r40/{}_{}_{}".format(class_name, diff, thresh)
    ...
    def get_mAP(self, precision, recall):
        ...
        """
        precisions, recall_spacing = self.get_sampled_precision_recall(
            precision, recall
        )
    ...
    def get_sampled_precision_recall(self, precision, recall):
        ...
        recall_spacing = [
            1.0 / (self.sample_points - 1) * i for i in range(1, self.sample_points)
        ]
        recall_spacing = list(
            filter(
                lambda recall: recall_range[0] <= recall <= recall_range[1],
                recall_spacing,
            )
        )
    ...
    def eval_metric(self, gt_annos, dt_annos, metric, overlap_thresholds):
        assert len(gt_annos) == len(
            dt_annos
        ), "Must provide a prediction for every ground truth sample"
        ...
        precision = np.zeros(
            [num_classes, num_difficulties, num_minoverlap, self.sample_points]
        )
        recall = np.zeros(
            [num_classes, num_difficulties, num_minoverlap, self.sample_points]
        )
        ...
        for class_idx in range(num_classes):
            for difficulty_idx in range(num_difficulties):
                (
                    ...
                    ignores_per_sample,
                    total_num_valid_gt,
                ) = self.prepare_data(gt_annos, dt_annos, class_idx, difficulty_idx)
                ...
                for thresh_idx, min_overlap in enumerate(overlap_thresholds):
                    ...
                    for shard_idx, num_samples_per_shard in enumerate(shards):
                        gt_datas_part = np.concatenate(
                            gt_data_list[idx : idx + num_samples_per_shard], 0
                        )
                        dt_datas_part = np.concatenate(
                            dt_data_list[idx : idx + num_samples_per_shard], 0
                        )
                        dc_datas_part = np.concatenate(
                            dontcares[idx : idx + num_samples_per_shard], 0
                        )
                        ignored_dets_part = np.concatenate(
                            ignored_dets[idx : idx + num_samples_per_shard], 0
                        )
                        ignored_gts_part = np.concatenate(
                            ignored_gts[idx : idx + num_samples_per_shard], 0
                        )
                    ...
                    for i in range(len(thresholds)):
                        recall[class_idx, difficulty_idx, thresh_idx, i] = pr[i, 0] / (
                            pr[i, 0] + pr[i, 2]
                        )
                        precision[class_idx, difficulty_idx, thresh_idx, i] = pr[
                            i, 0
                        ] / (pr[i, 0] + pr[i, 1])
        ...
        for gt_anno, dt_anno in zip(gt_annos, dt_annos):
            ...
            gt_list.append(
                np.concatenate(
                    [
                        gt_anno["bbox"],
                        gt_anno["rotation_y"][..., np.newaxis],
                        gt_anno["alpha"][..., np.newaxis],
                        gt_anno["dimensions"],
                    ],
                    1,
                )
    ...
    def calculate_match_degree_sharded(self, gt_annos, dt_annos, metric):
        ...
        for num_samples_per_shard in shards:
            ...
            if metric == "BEV_AP":
                ...
                shard_match = self.bev_box_overlap(dt_boxes, gt_boxes).astype(
                    np.float64
                )
...
def clean_kitti_data(
    gt_anno,
    dt_anno,
    current_class,
    difficulty,
    id_to_name,
    difficulty_level_to_params=None,
):
    ...
    for i in range(num_gt):
        ...
        if (
            (
                gt_anno["occluded"][i]
                > difficulty_level_to_params["max_occlusion"][difficulty]
            )
            or (
                gt_anno["truncated"][i]
                > difficulty_level_to_params["max_truncation"][difficulty]
            )
        ...
        if valid_class == 1 and not ignore_for_truncation_occlusion:
            ignored_gt.append(0)
            num_valid_gt += 1
        elif valid_class == 0 or (
            ignore_for_truncation_occlusion and (valid_class == 1)
        ):
            ignored_gt.append(1)
        else:
            ignored_gt.append(-1)
    ...
    for i in range(gt_size):
        ...
        for j in range(det_size):
            ...
            if (
                ...
                assert (
                    not compute_fp
                ), "For sanity, compute_fp shoudl be False if we are here"
                det_idx = j
...
def compute_statistics_jit(
    ...
    NO_DETECTION = np.finfo(np.float32).min
    tp, fp, fn, error_yaw, similarity, match_degree, scale_error, confidence_error = (
        0,
        0,
        0,
        0,
    ...
    for i in range(gt_size):
        ...
        # Matched prediction
            ...
            if compute_angular_metrics:
                delta_yaw[delta_idx] = abs(
                    angle_diff(float(gt_yaws[i]), float(dt_yaws[det_idx]), 2 * np.pi)
                )
