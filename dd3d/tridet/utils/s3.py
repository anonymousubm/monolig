    ...
    if ckpt_path.startswith("s3://") or ckpt_path.startswith("https://"):
        ...
        if ckpt_path.startswith("s3://"):
            if not aws_credential_is_available():
                raise ValueError(
                    "AWS credentials are undefined in environment variables."
                )
...
def s3_copy(source_path, target_path, verbose=True):
    ...
    success = False
    command_str = "aws s3 cp --acl bucket-owner-full-control {} {}".format(
...
def sync_output_dir_s3(cfg, output_dir=None):
    ...
    assert (
        os.path.commonprefix([output_dir, output_root]) == output_root
    ), f"{output_dir}, {output_root}"
    tar_output_dir = os.path.join(
        cfg.SYNC_OUTPUT_DIR_S3.ROOT_IN_S3, output_dir[len(output_root) + 1 :]
    )
    ...
    elif comm.get_local_rank() == 0 and os.path.exists(
        os.path.join(output_dir, "logs")
    ):
        # local master -- only sync the log files
        log_output_dir, log_tar_output_dir = os.path.join(
            output_dir, "logs"
        ), os.path.join(tar_output_dir, "logs")
