...
from tridet.utils.s3 import (
    aws_credential_is_available,
    maybe_download_ckpt_from_url,
    sync_output_dir_s3,
)
from tridet.utils.setup import setup_distributed
from tridet.utils.wandb import (
    derive_output_dir_from_wandb_id,
    init_wandb,
    wandb_credential_is_available,
)
...
class D2LoggerCallback(Callback):
    def on_run_start(self, config, **kwargs):  # pylint: disable=unused-argument
        ...
        setup_logger(
            log_output_dir, distributed_rank=rank, name="detectron2", abbrev_name="d2"
        )
